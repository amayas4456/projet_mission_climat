...
class Window(tk.Toplevel):
    ...
    def __init__(self, parent):
        ...
        self.title('Q3 : départements pour une région donnée (version statique)')
        ...
        for column in columns:
            self.treeView.column(column, anchor=tk.CENTER, width=150)  # Ajustez la largeur selon vos besoins
            self.treeView.heading(column, text=column)
        self.treeView.grid(columnspan=3, row=2, sticky='nswe')

        # Ajout du menu déroulant (Combobox)
        self.region_combobox = ttk.Combobox(self, values=self.get_region_names())
        self.region_combobox.grid(row=0, column=1)
        self.region_combobox.bind('<Return>', self.searchRegion)

    def get_region_names(self):
        # Fonction pour récupérer la liste des noms de régions depuis la base de données
        try:
            cursor = db.data.cursor()
            result = cursor.execute("SELECT DISTINCT nom_region FROM Regions ORDER BY nom_region").fetchall()
            return [row[0] for row in result]
        except Exception as e:
            print("Erreur lors de la récupération des noms de région :", repr(e))
            return []
